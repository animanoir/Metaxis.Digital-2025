...
const LatestEventsGrid = () => {
  ...
  return (
    <div className="w-full max-w-7xl mx-auto px-4 py-8">
      <h2 className="text-4xl font-bold text-gray-800 mb-8 font-karla">
        Upcoming Events
      </h2>
          ...
          <Link href={`/events/${event.slug}`} key={event.slug}>
            <article className="group relative overflow-hidden rounded-lg bg-white shadow-lg transition-all duration-300 hover:shadow-2xl hover:translate-y-[-4px] cursor-pointer">
              ...
              <div className="p-6 space-y-3">
                <h3 className="font-karla text-2xl text-gray-800 font-bold tracking-tight line-clamp-2">
                  {event.eventName}
                </h3>

                <p className="text-sm text-gray-600 font-lora line-clamp-3">
                  {event.description}
                </p>
